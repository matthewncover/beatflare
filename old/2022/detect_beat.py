...
sampling_rate = 22050 #11025 44100
...
def save_clip(stream_clip_size):
    ...
    while input_stream.is_active():
        ...
        if time.time() - start > 10:
            np.save("freestyle_beat.npy", stream_data_arr)
...
def print_beats(stream_clip_size, start):
    ...
    while input_stream.is_active():
        ...
        try:
            beat_midpoints, runtime_delta = find_beats(hop)
            # beat_midpoints = beat_midpoints / 25000
            beat_midpoints = beat_midpoints / sampling_rate

            for i, beat in enumerate(beat_midpoints):
                # time.sleep(beat / 25000)
                if i > 0:
                    time.sleep(beat - beat_midpoints[i-1] - runtime_delta)
                    # print(beat - beat_midpoints[i-1] - runtime_delta)
                    print(runtime_delta)
                else:
                    time.sleep(beat)
                    # print(beat)
...
if __name__ == "__main__":
    ...
    stream_clip_size = sampling_rate*10
    ...
    p3 = multiprocessing.Process(target=save_clip, args=[stream_clip_size])
